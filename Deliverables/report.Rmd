...
test_metrics <- function(augment, model_name) {
  bind_rows(
    roc_auc(augment,
            truth = Class,
            .pred_Tarp,
            event_level = 'second'),
    threshold_metrics(augment,
                      truth = Class,
                      estimate = .pred_class,
                      )
  ) %>%
...
knn_threshold_preds <- threshold_preds(knn_cv_preds, 0.04)
...
knn_train_train_cm <- conf_mat(knn_threshold_preds, truth = Class, estimate = .pred_class) ## !!
...
knn_test_preds <- threshold_preds(augment(knn_fit, new_data = data), 0.04)
