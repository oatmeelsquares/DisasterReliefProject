...
knn_train_train_cm <- conf_mat(knn_threshold_preds,
                               truth = Class,
                               estimate = .pred_class) ## !! update var name
...
knn_test_preds <- threshold_preds(augment(knn_fit, new_data = data),
                                  knn_threshold)
...
compare_cv_metrics <- bind_rows(log_cv_metrics %>% mutate(model = 'logistic regression'),
...
```{r table-2-metrics}
compare_cv_metrics %>% knitr::kable(caption = "Compare AUC of all models")
```
