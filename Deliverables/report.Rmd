...
data <- data %>% 
  mutate(Class = factor(ifelse(Class == 'Blue Tarp', 'Tarp', 'Non-Tarp')))
table(data$Class) %>% knitr::kable(caption = "Table 1: Count of Class")
```
...
```{r figure-X-pen-threshold-metrics, output = TRUE}
#| fig.align: center
#| out.width: 70%
...
```{r log-conf-mat, output = TRUE}
log_cm
```

```{r pen-conf-mat, output = TRUE}
pen_cm
```
...
```{r table-2-metrics, output = TRUE}
table(compare_metrics) %>% knitr::kable(caption = "Table 2: Compare AUC of all models")
```
...
```{r table-3-metrics, output = TRUE}
table(compare_threshold_metrics) %>% knitr::kable(caption = "Table 3: Compare Threshold Metrics of all models")
```
