...
knn_final <- knn_wf %>% 
  finalize_workflow(knn_best_params)              ## !! split to save final wf
...
knn_cv_preds <-  collect_predictions(knn_fitcv) ## update var name
knn_cv_metrics <- collect_metrics(knn_fitcv)    ## !!

# get ROC plot
knn_cv_roc <- roc_plot(knn_cv_preds, 'KNN')   ## !! add model name, change var name
...
knn_train_train_cm <- conf_mat(knn_threshold_preds, truth = Class, estimate = .pred_class) ## !! update var name
...
knn_test_preds <- threshold_preds(augment(knn_fit, new_data = data), knn_threshold)
...
lda_roc <- roc_plot(lda_cv_preds, 'LDA')
...
qda_roc <- roc_plot(qda_cv_preds, 'QDA')
...
# get ROC plot
log_roc <- roc_plot(log_cv_preds, 'Logistic Regression')
...
# get ROC plot
pen_roc <- roc_plot(pen_cv_preds, 'Penalized LR')
