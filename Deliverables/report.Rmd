...
# get best threshold                ##
knn_threshold <- knn_max_j[1, 1]    ## !! add
...
# get best threshold
lda_threshold <- lda_max_sens[1, 1]
...
## PART II ##
# fit final workflow to training data
lda_fit <- fit(lda_final, holdout)

# get predictions and metrics for holdout data
lda_test_preds <- threshold_preds(augment(lda_fit, new_data = holdout),
                                  lda_threshold)
lda_test_roc <- roc_plot(lda_test_preds)
...
# get best threshold
qda_threshold <- (qda_max_sens[1, 1] + qda_max_j[1, 1]) / 2
...
## PART II ##
# fit final workflow to training data
qda_fit <- fit(qda_final, holdout)

# get predictions and metrics for holdout data
qda_test_preds <- threshold_preds(augment(qda_fit, new_data = holdout),
                                  qda_threshold)
qda_test_roc <- roc_plot(qda_test_preds)
...
## PART II ##
# fit final workflow to training data
log_fit <- fit(log_final, holdout)

# get predictions and metrics for holdout data
log_test_preds <- threshold_preds(augment(log_fit, new_data = holdout),
                                  log_threshold)
log_test_roc <- roc_plot(log_test_preds)
...
## PART II ##
# fit final workflow to training data
pen_fit <- fit(pen_final, holdout)

# get predictions and metrics for holdout data
pen_test_preds <- threshold_preds(augment(pen_fit, new_data = holdout),
                                  pen_threshold)
pen_test_roc <- roc_plot(pen_test_preds)
...
cv_rocs <- ( log_cv_roc + pen_cv_roc  + knn_cv_roc) / ( lda_cv_roc + qda_cv_roc )

compare_cv_metrics <- bind_rows(
  log_cv_metrics %>% mutate(model = 'logistic regression'),
  pen_cv_metrics %>% mutate(model = 'penalized logreg'),
...
compare_threshold_metrics <- bind_rows(
  log_threshold_metrics %>% mutate(model = 'logistic regression'),
...
test_rocs <- ( log_test_roc + pen_test_roc  + knn_test_roc) / ( lda_test_roc + qda_test_roc )

compare_test_metrics <- bind_rows(
  log_test_metrics %>% mutate(model = 'logistic regression'),
  pen_test_metrics %>% mutate(model = 'penalized logreg'),
