...
Support vector machines are models which frequently perform well in a wide variety of applications and are worth considering for this analysis. We analyzed support vector machines with three kernel types: linear, polynomial, and radial. Support vector machines have various tuning parameters to consider depending on the kernel chosen. The first parameter that is tuned for all models is cost, which represents a measure of how much leeway is given for observations to be on the incorrect side of the hyperplane. A larger C value creates a model with higher bias but lower variance and vice versa. Our next parameter is insensitivity margin, this indicates the value of the epsilon in the loss function. This value effectively determines the width of our area around the hyperplane for values that will be penalized. The polynomial kernel has a degree parameter that signifies the order of the kernel function (i.e. 2nd degree vs 3rd degree). The sigma in the radial function determines equates to standard deviation and represents how the decision boundary is drawn. A larger sigma will reduce variance but increase bias and vice versa.

Our Linear SVM performed best with a cost of 31.49 and a margin of .14 This model allows for more flexibility which makes sense as the data set is one that likely does not fit a linear decision area. These parameters allow for the model to attempt to address this draw back and allow for more misclassifications of values. The Polynomial SVM highlights the potential deficiencies with the linear model when examining the parameter tuning. The lowest performing is a 1st degree (linear model) followed by a second and a 3rd degree as the best model. The optimal cost is .1 and the optimal margin is .003. This model has a much smaller cost and narrower margin relative to the linear model which creates a model with relatively higher variance but lower bias. These parameters also reflect how the data is likely to be non-linear in nature. The Radial SVM has a cost of 25.52, a margin of .13 and a Sigma of .006. The cost and margin here are more in line with the linear model though as Figures 17 and 19 highlight, these parameters are relatively consistent throughout the range. This allows the radial function to be slightly more biased to the dataset and outperform when it comes to classifications.

The two threshold metrics to evaluate for these models are the J-Index and sensitivity. These measures represent two different approaches to optimize the model with the j-index being a more balance approach that takes into account both sensitivity and specificity. The three thresholds for each model are 0.06, 0.03, and 0.02 for the linear, polynomial, and radial SVM models respectively. For optimization purposes we utilize the j-index as the metric to select the threshold. This allows us to approach the final problem with more flexibility depending on our final objective constraints with false positives and false negatives.
...
compare_threshold_metrics <- bind_rows(
  ...
  mutate(precision = c(0.8729, 0.5824, 0.5380, 0.7152, 0.6743, 0.7631, 0.6981, 0.7606, 0.7206))
...
compare_test_metrics <- bind_rows(
  ...
  rename(sensitivity = "specificity", # Renaming due to error with how R is calculating levels
         specificity = "sensitivity") %>% 
  mutate(precision = c(0.2998, 0.1789, 0.1591, 0.0696, 0.0656, 0.1769,0.0660, 0.1243, 0.0784))
...
```{r}
# 13452/(13452+31424) #KNN
# 13860/(13860+63633) #LDA
# 13586/(13586+71822) #QDA
# 14479/(14479+193475) #Log
# 14480/(14480+206144) #Pen
# 13863/(13863+64505) #Ens
# 14478/(14478+204855) #SVML
# 14480/(14480+101993) #SVMP
# 14480/(14480+170042) #SVMR
```
```{r}
# 1999/(1999+291) #KNN
# 1791/(1791+1284) #LDA
...
**discussion or analysis justifying why your findings above are compatible or reconcilable.**
